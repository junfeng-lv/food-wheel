...
if (-not [System.IO.Path]::IsPathRooted($script)) { $script = Join-Path (Get-Location) $script }
$root = [System.IO.Path]::GetFullPath((Split-Path $script))
$stage = [System.IO.Path]::GetFullPath((Join-Path $env:SystemDrive "codex_food_wheel_build"))
$expectedStage = [System.IO.Path]::GetFullPath((Join-Path $env:SystemDrive "codex_food_wheel_build"))
...
if (-not (Test-Path $keystoreStage)) {
    if (Test-Path $keystoreRoot) {
        ...
        if (Test-Path $keystoreStage) { Copy-Item -LiteralPath $keystoreStage -Destination $keystoreRoot }
    }
}
...
$apkName = "今天吃什么_v1.0.apk"
Copy-Item -LiteralPath "$build\signed.apk" -Destination "$root\$apkName" -Force
New-Item -ItemType Directory -Force "$root\dist" | Out-Null
Copy-Item -LiteralPath "$build\signed.apk" -Destination "$root\dist\$apkName" -Force
$info = Get-Item "$root\dist\$apkName"
Write-Host "BUILD OK: $($info.FullName) ($($info.Length) bytes)"
if (Test-Path -LiteralPath $stage) {
    $r = [System.IO.Path]::GetFullPath($stage)
    if ($r -eq $expectedStage) { Remove-Item -LiteralPath $r -Recurse -Force -ErrorAction SilentlyContinue }
}
